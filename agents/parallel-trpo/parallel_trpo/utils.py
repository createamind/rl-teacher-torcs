...
import numpy as np
import scipy.signal
from autodrive.model.selu import selu
...
def make_network(network_name, input_layer, hidden_size, action_size):
    with tf.variable_scope(network_name):
        ...
        h2, h2_vars = make_fully_connected("policy_h2", h1, hidden_size, weight_init=tf.truncated_normal_initializer(stddev=0.01))
        h3, h3_vars = make_fully_connected("policy_h3", h2, hidden_size, weight_init=tf.truncated_normal_initializer(stddev=0.01))
        h4, h4_vars = make_fully_connected("policy_h4", h3, hidden_size, weight_init=tf.truncated_normal_initializer(stddev=0.01))
        h5, h5_vars = make_fully_connected("policy_h5", h4, hidden_size, weight_init=tf.truncated_normal_initializer(stddev=0.01))
        h6, h6_vars = make_fully_connected("policy_h6", h5, hidden_size, weight_init=tf.truncated_normal_initializer(stddev=0.01))
        h7, h7_vars = make_fully_connected("policy_h7", h6, hidden_size, weight_init=tf.truncated_normal_initializer(stddev=0.01))
        h8, h8_vars = make_fully_connected("policy_h8", h7, action_size, weight_init=tf.truncated_normal_initializer(stddev=0.01))


        logstd_action_dist_param = tf.Variable(
            (.01 * np.random.randn(1, action_size)).astype(np.float32), name="policy_logstd")

    variable_list = h1_vars + h2_vars + h3_vars +h4_vars+h5_vars+h6_vars+h7_vars+h8_vars+ [logstd_action_dist_param]
    # means for each action
    avg_action_dist = h8+1e-10
