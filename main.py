...
class Model(ModelBase):
    ...
    def _build_ad_nn(self, tensor_io):
        ...
        with tf.variable_scope('actor', reuse=nnc.reuse) as vs:
            l = tf.stop_gradient(l)
            mu_steering = 1. * tf.layers.dense(l, 1, activation=tf.nn.tanh, name='fc-mu-steering')
            mu_accel = tf.layers.dense(l, 1, activation=tf.nn.tanh, name='fc-mu-accel')
            mus = tf.concat([mu_steering, mu_accel], axis=-1)
            # mus = tf.layers.dense(l, 2, activation=tf.nn.tanh, name='fc-mus')
            # sigmas = tf.layers.dense(l, 2, activation=tf.nn.softplus, name='fc-sigmas')
            ...
            sigma_steering_ = 1. * tf.layers.dense(l, 1, activation=tf.nn.sigmoid, name='fc-sigma-steering')
            sigma_accel_ = 1. * tf.layers.dense(l, 1, activation=tf.nn.sigmoid, name='fc-sigma-accel')
            sigma_beta_steering = symbf.get_scalar_var('sigma_beta_steering', 0.3, summary=True, trainable=False)
            sigma_beta_accel = symbf.get_scalar_var('sigma_beta_accel', 0.3, summary=True, trainable=False)
            ...
            # sigma_steering = sigma_steering_
            # sigma_accel = sigma_accel_
            sigmas = tf.clip_by_value(tf.concat([sigma_steering, sigma_accel], axis=-1), 0., 1.)
            #     sigma_steering = tf.clip_by_value(sigma_steering, 0.1, 0.5)
    ...
    def _build_graph(self):
        ...
        for eidx, evaluator in enumerate(evaluators):
            with NNContext(evaluator.name, device='/device:GPU:%d' % gpu_towers[eidx%len(gpu_towers)], add_variable_scope=True, summary_collection=True):
...
if __name__ == '__main__':
    ...
    if args['train']:
        ...
        evaluators = [
            Evaluator(data_io, 'evaluate/valid', batch_size=4, is_training=False, cls_pool=clsPool),
        ]
